 ...
 */
async function initializeLSPConnection(ws, state, containerId, workspacePath, cleanup) {
    try {
        // Inspect container
        ...
        const exec = await (0, docker_1.createRustAnalyzerExec)(containerId, workspacePath);
        if (!state.connectionActive)
            return;
        ...
        const { connection, stderr } = (0, lsp_connection_1.createLSPConnection)(execStream);
        state.lspConnection = connection;
        // Set up notification handlers
        (0, lsp_connection_1.setupNotificationHandlers)(connection, ws, state, containerId);
        // Set up stream handlers
        (0, lsp_connection_1.setupStreamHandlers)(execStream, stderr, ws, state, containerId);
