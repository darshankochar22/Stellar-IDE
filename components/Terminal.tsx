...
export default function Terminal({ isOpen, onClose, logs, height: initialHeight = 250, onHeightChange }: TerminalProps) {
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const maxTerminalHeight = window.innerHeight - topBarHeight - minEditorHeight;
      
      const newHeight = Math.max(0, window.innerHeight - e.clientY - topBarHeight);
  ...
  return (
    <div className="flex flex-col bg-[#171717]" style={{ height: `${height}px` }}>
      {/* Resize Handle */}
      <div
        onMouseDown={handleMouseDown}
        className={`h-1 bg-[#252525] hover:bg-[#3a3a3a] cursor-row-resize transition-colors ${isDragging ? 'bg-[#3a3a3a]' : ''}`}
      ...
      <div className="h-10 bg-[#171717] flex items-center justify-between px-4 flex-shrink-0">
        <div className="flex items-center gap-3">
          <span className="text-xs text-gray-400 font-semibold uppercase">Console</span>
        ...
        {logs.length === 0 ? (
          ...
          logs.map(log => (
            <div key={log.id} className="flex gap-3">
              ...
              <span className={`${getLogColor(log.type)} text-xs flex-1 break-all`}>
                {log.message}
              </span>
            </div>
          ))
